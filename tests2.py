        ...
        try:
            ...
            if self.model_provider == "huggingface":
                ...
                if self.model_name == "openai/whisper-large":
                    ...
                    self.model.config.forced_decoder_ids = None  # clear forced_decoder_ids

                    # Load and preprocess audio
                    waveform, sample_rate = torchaudio.load(aud)  # shape: (channels, samples)
                    ...
                    if waveform.shape[0] > 1:
                        waveform = waveform.mean(dim=0)  # shape: (samples,)

                    # Convert PyTorch tensor to numpy array (1D)
                    audio_np = waveform.cpu().numpy()

                    # Prepare inputs for processor: pass 1D numpy array + sample rate
...
if __name__ == "__main__":
    ...
    wrapper = ModelWrapper(provider=provider, model_name=model_name, task=task)
    
    # Load the model (from Azure if available, else from HuggingFace)
    wrapper.load_model()
    ...
    # # Run inference
    # output = wrapper.run_inference(input_data=input_data, task=task)
