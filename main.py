...
async def pull_model(
    provider: Optional[str] = Form(
        None, description="Model provider, e.g., 'huggingface'"
    ),
    task: Optional[
        Literal[
    ...
    ] = Form(None, description="Task type"),
    model_identifier: Optional[str] = Form(
    ...
    try:
        ...
        load_msg = f"Model '{model_identifier}' {'' if result else 'not'} loaded successfully from {provider}."
