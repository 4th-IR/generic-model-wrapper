        ...
        if provider == "huggingface":
            ...
            try:
                if self.model_category == "vision":
                    self.model = AutoModelForImageClassification.from_pretrained(temp_model_path)
                    self.image_processor = AutoImageProcessor.from_pretrained(temp_model_path)
                    self.pipeline = pipeline(self.task, model=self.model, image_processor=self.image_processor)
                
                elif self.model_category == "text":
                ...
                elif self.model_category == "audio":
                    self.model = AutoModelForAudioClassification.from_pretrained(temp_model_path)
                    self.audio_processor = AutoProcessor.from_pretrained(temp_model_path)
                    self.pipeline = pipeline(self.task, model=self.model, processor=self.audio_processor)

                elif self.model_category == "multimodal":
                    self.multimodal_processor = AutoProcessor.from_pretrained(temp_model_path)
                    self.model = AutoModel.from_pretrained(temp_model_path)
                    self.pipeline = pipeline(self.task, model=self.model, processor=self.multimodal_processor)

                else:
                    raise ValueError(f"Unsupported model category: {self.model_category}")
...
if __name__ == "__main__":
    ...
    model_wrapper = ModelWrapper(model_name="AI-Enthusiast11/pii-entity-extractor", 
